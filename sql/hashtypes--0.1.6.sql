...
CREATE FUNCTION sha1b(bytea) RETURNS sha1 LANGUAGE C STRICT AS 'hashtypes', 'byteasha1';
CREATE FUNCTION bytea(sha1) RETURNS bytea LANGUAGE C STRICT AS 'hashtypes', 'shabytea1';
CREATE CAST (bytea AS sha1) WITH FUNCTION sha1b(bytea) AS ASSIGNMENT;
...
CREATE TYPE sha224 ( INPUT = sha224_in, OUTPUT = sha224_out, 
	receive = sha224_recv, send = sha224_send,
	INTERNALLENGTH = 28);
...
CREATE FUNCTION sha224b(bytea) RETURNS sha224 LANGUAGE C STRICT AS 'hashtypes', 'byteasha224';
CREATE FUNCTION bytea(sha224) RETURNS bytea LANGUAGE C STRICT AS 'hashtypes', 'shabytea224';
CREATE CAST (bytea AS sha224) WITH FUNCTION sha224b(bytea) AS ASSIGNMENT;
...
CREATE TYPE sha256 ( INPUT = sha256_in, OUTPUT = sha256_out, 
	receive = sha256_recv, send = sha256_send,
	INTERNALLENGTH = 32);
...
CREATE FUNCTION sha256b(bytea) RETURNS sha256 LANGUAGE C STRICT AS 'hashtypes', 'byteasha256';
CREATE FUNCTION bytea(sha256) RETURNS bytea LANGUAGE C STRICT AS 'hashtypes', 'shabytea256';
CREATE CAST (bytea AS sha256) WITH FUNCTION sha256b(bytea) AS ASSIGNMENT;
...
CREATE TYPE sha384 ( INPUT = sha384_in, OUTPUT = sha384_out, 
	receive = sha384_recv, send = sha384_send,
	INTERNALLENGTH = 48);
...
CREATE FUNCTION sha384b(bytea) RETURNS sha384 LANGUAGE C STRICT AS 'hashtypes', 'byteasha384';
CREATE FUNCTION bytea(sha384) RETURNS bytea LANGUAGE C STRICT AS 'hashtypes', 'shabytea384';
CREATE CAST (bytea AS sha384) WITH FUNCTION sha384b(bytea) AS ASSIGNMENT;
...
create function sha512_send( sha512 ) returns bytea immutable 
	language c strict as 'hashtypes', 'sha_send512';
create function sha512_recv( internal ) returns sha512 immutable
	language c strict as 'hashtypes', 'sha_recv512';
CREATE TYPE sha512 ( INPUT = sha512_in, OUTPUT = sha512_out, 
	receive = sha512_recv, send = sha512_send,
	INTERNALLENGTH = 64);
...
CREATE FUNCTION sha512b(bytea) RETURNS sha512 LANGUAGE C STRICT AS 'hashtypes', 'byteasha512';
CREATE FUNCTION bytea(sha512) RETURNS bytea LANGUAGE C STRICT AS 'hashtypes', 'shabytea512';
CREATE CAST (bytea AS sha512) WITH FUNCTION sha512b(bytea) AS ASSIGNMENT;
